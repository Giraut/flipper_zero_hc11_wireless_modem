...
void load_configuration(App *app) {
  ...
  if(channel_idx >= nb_config_channel_values) {
    FURI_LOG_I(TAG, "Invalid channel option %d in config file %s",
			read_config.channel, config_file);
    return;
  }
  ...
  if(address_idx >= nb_config_address_values) {
    FURI_LOG_I(TAG, "Invalid address option %d in config file %s",
			read_config.address, config_file);
    return;
  }
  ...
  if(txpower_idx >= nb_config_txpower_values) {
    FURI_LOG_I(TAG, "Invalid TX power option %d in config file %s",
			read_config.txpower, config_file);
    return;
  }
  ...
  if(mode_idx >= nb_config_mode_values) {
    FURI_LOG_I(TAG, "Invalid mode option %d in config file %s",
  ...
  for(passthru_chan_idx = 0;
	...
	read_config.passthru_chan !=
			config_passthru_chan_values[passthru_chan_idx];
	passthru_chan_idx++);

  if(passthru_chan_idx >= nb_config_passthru_chan_values) {
    FURI_LOG_I(TAG, "Invalid USB passthrough channel option %d in config file "
			"%s", read_config.passthru_chan, config_file);
    return;
  }
  ...
  variable_item_set_current_value_text(app->item_channel,
					config_channel_names[channel_idx]);
  FURI_LOG_I(TAG, "  %s: %s", config_channel_label,
			config_channel_names[channel_idx]);
  ...
  variable_item_set_current_value_text(app->item_address,
					config_address_names[address_idx]);
  FURI_LOG_I(TAG, "  %s: %s", config_address_label,
			config_address_names[address_idx]);
  ...
  variable_item_set_current_value_text(app->item_txpower,
					config_txpower_names[txpower_idx]);
  FURI_LOG_I(TAG, "  %s: %s", config_txpower_label,
			config_txpower_names[txpower_idx]);
  ...
  variable_item_set_current_value_index(app->item_passthru_chan,
					passthru_chan_idx);
  variable_item_set_current_value_text(app->item_passthru_chan,
				config_passthru_chan_names[passthru_chan_idx]);
  FURI_LOG_I(TAG, "  %s: %s",
