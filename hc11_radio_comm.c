...
static uint8_t cc1101_regs[] = {
    ...
    /* Channel number */
    CC1101_CHANNR, 0x02,	/* Default channel */

    /* Device address */
    CC1101_ADDR, 0x00,		/* Default address */
...
//    CC1101_PKTCTRL1, 0x4d,
    CC1101_PKTCTRL1, 0x45,	// Don't set CRC_AUTOFLUSH
    ...
    0x34, 0x3a, 0x3b, 0x36, 0xf5, 0x00, 0x16, 0x01
};
...
void set_raw_data_handler(HC11RadioCommApp *app,
...
static void cc1101_gdo0_int_callback(void *ctx) {
  ...
  furi_thread_flags_set(furi_thread_get_id(app->hc11_rx_tx_thread),
			furi_hal_gpio_read(&gpio_cc1101_g0)? gdo0_rise :
								gdo0_fall);
}
...
static int32_t hc11_rx_tx_thread(void *ctx) {
  ...
  while(1) {
    ...
    /* Get events */
    evts = furi_thread_flags_wait(stop | set_cc1101 | unset_cc1101 |
					gdo0_rise | gdo0_fall | send_data,
    ...
    if((evts & stop || evts & unset_cc1101) && app->radio_started) {
      ...
      /* Reset the GDO line */
      furi_hal_gpio_init(&gpio_cc1101_g0, GpioModeAnalog, GpioPullNo,
				GpioSpeedLow);
    ...
    if((evts & set_cc1101) && !app->radio_started) {
      ...
      FURI_LOG_D(TAG, "CC1101 patable[0]: 0x%02x",
			cc1101_regs[sizeof(cc1101_regs) - 8]);
      ...
      furi_hal_gpio_init(&gpio_cc1101_g0, GpioModeInterruptRiseFall,
				GpioPullNo, GpioSpeedLow);

      /* Set the GDO0 interrupt callback */
      furi_hal_gpio_add_int_callback(&gpio_cc1101_g0, cc1101_gdo0_int_callback,
					(void *)app);
    ...
    if(evts & gdo0_rise && app->receiving) {
      ...
      while(leave_bytes_in_fifo) {
        ...
        while(pkt_bytes_remaining && num_rxbytes > leave_bytes_in_fifo) {
          cc1101_read_reg(spi, CC1101_FIFO, app->cc1101_rx_buf_pkt +
      ...
      /* Log the received packet in the traces log. Only do this if the trace
         ...
         it normally isn't */
      if(loglevel == FuriLogLevelTrace) {
        snprintf(app->spstr, sizeof(app->spstr),
			"Recv %d bytes - %sRSSI %03.1fdbm, LQI %d: ",
			app->cc1101_rx_buf_pkt_size, crc_ok? "" : "CRC NOK!, ",
			(double)app->rssi, app->lqi);
        l = strlen(app->spstr);
        for(i = 0; i < app->cc1101_rx_buf_pkt_size; i++)
          snprintf(app->spstr + l + (i * 3), 4, "%02X ",
			app->cc1101_rx_buf_pkt[i]);
      ...
      switch(app->hc11_mode) {
        ...
        case 1:	/* FU1 */
          ...
          l = app->cc1101_rx_buf_pkt_size == 57? app->cc1101_rx_buf_pkt[56] :
				app->cc1101_rx_buf_pkt_size == 56? 55 : 0;

          if(l && l <= 55 && app->cc1101_rx_buf_pkt[0] == app->cc1101_addr) {

            /* Send the packet to the main thread */
            furi_stream_buffer_send(app->rx_stream, app->cc1101_rx_buf_pkt + 1,
					l, FuriWaitForever);
          }
		...
		app->cc1101_rx_buf_pkt[1] == 0x44) {
            ...
            /* Send the packet to the main thread */
            furi_stream_buffer_send(app->rx_stream, app->cc1101_rx_buf_pkt + 2,
    ...
    if((evts & send_data && app->receiving) ||
	(evts & gdo0_fall && !app->receiving)) {
      ...
      /* We have data to send */
        ...
        switch(app->hc11_mode) {
          ...
          case 1:	/* FU1 */
            ...
            /* Send actual data */
              ...
              if(pkt_bytes_remaining < 55) {
                ...
                app->cc1101_tx_buf_pkt[1] = app->cc1101_addr;
                memcpy(app->cc1101_tx_buf_pkt + 2,
			app->tx_buf + app->tx_buf_i, pkt_bytes_remaining);
          ...
          case 3: /* FU3 */
            ...
            if(pkt_bytes_remaining < l) {
              ...
              memcpy(app->cc1101_tx_buf_pkt + 3,
			app->tx_buf + app->tx_buf_i, pkt_bytes_remaining);
              app->tx_buf_i = app->tx_buf_size;
        ...
        if(loglevel == FuriLogLevelTrace) {
          snprintf(app->spstr, sizeof(app->spstr),
			"Sent %d bytes: ", app->cc1101_tx_buf_pkt_size - 1);
          l = strlen(app->spstr);
          for(i = 1; i < app->cc1101_tx_buf_pkt_size; i++)
            snprintf(app->spstr + l + (i * 3) - 3, 4, "%02X ",
			app->cc1101_tx_buf_pkt[i]);
          FURI_LOG_I(TAG, app->spstr);
        }
...
static int32_t main_rx_thread(void *ctx) {
  ...
  while(1) {
    ...
    nb_bytes = furi_stream_buffer_receive(app->rx_stream, app->rx_buf,
						RX_BUF_SIZE, 250);
...
/** Start the radio **/
void start_radio(HC11RadioCommApp *app, uint8_t channel, uint8_t address,
					uint8_t txpower, uint8_t mode) {
